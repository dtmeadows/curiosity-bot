...
async function handleMessageContent(content, messageServerId, messageAuthor) {
  ...
  const outputMessage = await matchingCommand.execute(
    contentAfterCommand,
    messageServerId,
    messageAuthor,
  );

  if (commandName.endsWith('boat')) {
